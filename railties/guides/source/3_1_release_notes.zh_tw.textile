...
* 可逆的資料庫遷移
* Assets Pipeline
* jQuery 成為預設的 JavaScript 函式庫

本篇發佈重點說明中，包含主要的升級變動，但是並沒有詳述所有的小錯誤修正與小變動。如果你想要看所有發佈細節，請到 GitHub 上 Rails 專案目錄的 "提交列表":https://github.com/rails/rails/commits/master 閱讀。

endprologue.

h3. 升級到 Rails 3.1

如果你是升級一個已經存在的應用程式，先擁有良好覆蓋率的測試再進行升級，是個比較妥當的規劃。在升級 Rails 3.1 之前，你應該先升級到 Rails 3.0 ，並且確保你的應用程式依然可以正確無誤地運行。然後依序小心地確認以下幾點變動：

h4. Rails 3.1 運行的最低需求是 Ruby 1.8.7

Rails 3.1 要求在 Ruby 1.8.7 或是更高的版號上才能運行，正式宣告放棄支援 Ruby 1.8.6 或是更早的版本，使用者請盡早做 Ruby 升級的動作。 Rails 3.1 也兼容於 Ruby 1.9.2 上運行。

TIP: 注意到 Ruby 1.8.7 p248 和 p249 有編組錯誤(marshaling bugs)，會造成 Rails 意外結束。 Ruby Enterprise Edition 已經在版號 1.8.7-2010.02 修正了這個錯誤。在 Ruby 1.9 的部份，由於 Ruby 1.9.1 會出現徹底地 segmentation fault ，因而無法使用。想要用 Ruby 1.9 的使用者，請直接安裝 1.9.2 就可以順利的運行。

h3. 創建 Rails 3.1 應用程式

<shell>
# 你必須先安裝好 'rails' rubygem
$ rails new myapp
$ cd myapp
</shell>
