...
h3. Active Record 與 Referential Integrity

所謂的 Active Record 之道，認為有頭腦的應該是 models（模型），而不是資料庫。所以，一些把動腦工作推回去給資料庫的功能，像是觸發（triggers）或外部鍵約束（foreign key constraints）這類的，就不會常常去用到。

要確保資料完整的話，在 model 裡面，可以用 +validates_uniqueness_of+ 來做驗證功能。The +:dependent+ option on associations allows models to automatically destroy child 物件s when the parent is destroyed. Like anything which operates at the 應用程式 level these cannot guarantee referential integrity and so some people augment them with foreign key constraints.
...
* Augest ??, 2010: 翻譯完成 by "ETBlue":http://etblue.blogspot.com
...
技術指導：張文鈿 (a.k.a. "ihower":http://ihower.tw/blog/)
意見回饋：高見龍 (a.k.a. "Eddie Kao":http://blog.eddie.com.tw/)
翻譯的原始碼位於 "http://github.com/ihower/docrails":http://github.com/ETBlue/docrails
