...
* 了解 Active Record 物件的生命週期
* 使用內建的 Active Record 驗證輔助工具
* 打造您自訂的驗證方法
* 處理驗證流程所產生的錯誤訊息
* 撰寫回呼方法，對應物件生命週期中所發生的事件
* 撰寫特殊的類別，封裝回呼之間的共通行為
* 建立觀察者 (Observers)，在原類別外部回應物件生命週期中所發生的的事件

endprologue.

h3. 物件生命週期 (Object Life Cycle)

在 Rails 正常運作期間，會產生一些物件，也會更新、銷毀它們。為了方便您操控應用程式、操控資料，Active Record 提供了 <em>物件生命週期</em> 內的掛載點(hooks)。
...
新記錄 (record) 的新增與儲存，會對資料庫送出 SQL 的 +INSERT+ 操作 (operation) 。現有記錄的更新，則會送出 +UPDATE+ 操作。通常，命令送到資料庫之前，會進行驗證，要是任一個驗證失敗，這個物件就標記為無效 (invalid) ，於是 Active Record 就不會執行 +INSERT+ 或 +UPDATE+ ，也就不會讓無效的物件存進資料庫。您也可以針對物件的新增、儲存、更新，指派某幾個特定的驗證。
...
其中，方法名稱附有驚嘆號的像是 +save!+ ，會在這筆紀錄 invalid 的時候丟出例外 (raise an exception) 。至於名稱中沒有驚嘆號的，則不會丟出例外，像是 +save+ 跟 +update_attributes+ 會傳回 +false+ ，而 +create+ 跟 +update+ 則單純的傳回物件本身。
...
WARNING. +after_save+ 在建立、更新時，都會運作，但是，不論宣告的順序為何，+after_save+ 永遠在更特定的 +after_create+ 跟 +after_update+ 的 _之後_ 才會執行。
...
+after_initialize+ 和 +after_find+ 跟其他的回呼有些不同。它們沒有相對的 +before_*+ 回呼，而它們註冊的方式，只有一種，就是將它們定義為一般的方法 (regular methods) 。如果，用巨集風格的類別方法 (macro-style class methods) 來註冊 +after_initialize+ 或 +after_find+ ，那麼它們會直接被忽略。設計這樣的行為，是為了效能上的考量，因為，在資料庫中每找到一筆記錄，都會呼叫 +after_initialize+ 和 +after_find+ ，而明顯拖慢查詢的速度。
...
整個回呼鏈 (callback chain) 會被打包在一個交易功能 (transaction) 中，如果任一個  <em>before</em> 回呼方法傳回了 +false+ ，或者丟出了例外 (raises an exception) ，整個執行鏈 (execution chain) 就會中途停止 (halted) ，且會發送出一個資料庫  ROLLBACK。至於 <em>after</em> 回呼方法，則是只有在丟出例外的時候，才會導致執行鏈的中止。

WARNING. 丟出任何例外 (exception) 都會破壞 save 和回呼鏈的執行。除了 +ActiveRecord::Rollback+ 這個例外之外，它會精確地時告訴 Active Record 只進行滾回(rollback)動作。這個例外(exception) 會被 ActiveRecord 內部所捕捉而不會繼續往外丟出。
...
和驗證一樣，回呼也可以是條件式的，在某個先決條件成立時，才呼叫它。條件式可以用 +:if+ 跟 +:unless+ 選項來做，選項可以接收符號 (symbol) 、字串 (string) 、或 +Proc+ 物件。在某狀況下 *必須* 呼叫回呼，可以用 +:if+ 選項；在某狀況下 *必須不* 呼叫回呼，則是用 +:unless+ 選項。
...
h4. 註冊觀察者 (Registering Observers)
...
h3. 交易功能回呼 (Transaction Callbacks)

資料庫交易完成時，會觸發兩個額外的回呼，分別是 +after_commit+ 以及 +after_rollback+ 。它們跟 +after_save+ 回呼很像，不過，它們僅在資料庫變更提交 (committed) 或滾回 (rolled back) 之後，才會執行。當 active record models 要跟不在交易功能內的外部系統互動，這兩個回呼會很有用。

前面範例中，+PictureFile+ model 要在一筆紀錄銷毀後，跟著刪掉一個檔案。如果，在呼叫 +after_destroy+ 回呼之後，出現例外，導致交易功能滾回的話，刪掉的檔案並不會跟著回復，最後 model 會處在矛盾狀態中。下面的程式碼中，假設 +picture_file_2+ 無效，而 +save!+ 方法丟出了例外：
...
在交易功能區塊 (transaction block) 內建立、更新、或銷毀的 models ，必定會呼叫 +after_commit+ 與 +after_rollback+ 回呼。它們丟出的例外，都會被忽略，以免影響到其他的回呼。所以，如果您的 +after_commit+ 與 +after_rollback+ 回呼可能會丟出例外，必須在回呼之內救援 (rescue) 並適當的處理它。
...
部份翻譯詞彙參考自 "Ruby Programming－向Ruby之父學程式設計":http://anobii.com/books/Ruby_Programming/9789575279776/018bca897e4b3aafa8/ 以及 "松本行弘的程式世界：成為一流程式設計師的14種思考術":http://anobii.com/books/%E6%9D%BE%E6%9C%AC%E8%A1%8C%E5%BC%98%E7%9A%84%E7%A8%8B%E5%BC%8F%E4%B8%96%E7%95%8C/9789862013571/017262b33d6647d8f3/ 二書，皆為博碩出版。test02
