...
+validates_associated+ 預設的錯誤訊息是 "_is invalid_" 。每個相關聯的物件 (associated object) 都有自己的 +errors+ 集合，自己的錯誤訊息，只會出現在自己的集合裡面，不會跑到對方的模型 (calling model) 裡面。
...
送出 (submit) 上述的表單時，如果表單內有空白欄位 (empty fields) ，就會得到如下圖般的回應。預設狀況下，是沒有套用樣式 (styles) 的。
...
??? 預設範例的鷹架 (scaffolding) 會產生出 +public/stylesheets/scaffold.css+ ，這個樣式表 (CSS) 定義了如上圖這般的紅色系樣式。

要修改樣式表裡面所設定的 class 跟 id 的名稱，可以用 +:class+ 跟 +:id+ 這兩個選項，這在 +form_for+ 和 +error_messages_for+ 兩種輔助工具中，都是可以用的。
...
回呼 (callbacks) 是在物件生命週期裡的特定時刻，所被呼叫的方法。有了它，就可以在 Active Record 物件新增、儲存、更新、刪除、驗證、或者從資料庫讀出時，執行我們自己撰寫的程式碼。
...
macro-style class methods 也可以接收程式區塊 (block) ，如果區塊內的程式碼很短，短到可以一行之內寫完，那麼就可以考慮用區塊的方式來註冊回呼。
...
預設的狀況下，回呼方法 (callback methods) 是 public 的。但比較好的作法，是將它宣告為 protected 或 private ，因為，這樣做的話，在模型 (model) 之外的地方，就無法呼叫這些方法，也就不會侵犯到物件封裝 (object encapsulation) 的原則了。
...
Active Record 物件初始化的時候，不管是直接用 +new+ 初始化，或是從資料庫中載入一筆記錄 (record) ，都會呼叫 +after_initialize+ 這個回呼 (callback) 。它的好處是，可以避免直接覆蓋 (override) 掉 Active Record 原本的 +initialize+ 方法。

Active Record 從資料庫中載入一筆記錄 (record) 時，都會呼叫 +after_find+ 這個回呼 (callback) 。如果 +after_find+ 和 +after_initialize+ 兩者都有定義的話，則會先呼叫 +after_find+ 。
...
為模型註冊新的回呼 (callbacks) 時，這些回呼，會 queued 等待執行。queue 裡面包含：該模型全部的驗證 (validations) 、要註冊的回呼、以及要執行的資料庫操作 (database operation) 。

這一整個回呼鏈 (callback chain) ，是包裹在交易功能 (transaction) 裡面的。如果有任何  <em>before</em> 回呼方法傳回了 +false+ ，或者是丟出了例外 (raises an exception) ，整個執行鏈 (execution chain) 就會中途停止，且必須 issued 滾回 (ROLLBACK) 。??? 至於 <em>after</em> 回呼方法，則是只有在丟出例外 (raising an exception) 的時候，才會中止執行鏈。

WARNING. 預期會有 +save+ 動作的程式碼，以及 ??? 的程式碼，一旦遇到任何的例外 (exception) ，都可能會被打斷。 Raising an arbitrary exception may break code that expects +save+ and friends not to fail like that. 進行滾回 (rollback) 的時候， +ActiveRecord::Rollback+ 這個例外，會精確的告訴 Active Record 有個滾回正在進行。這個例外，會由內部捕捉 (internally captured) ，但不會被 ??? 再次丟出 (reraised) 。

h3. 關聯性回呼 (Relational Callbacks)

回呼 (callbacks) 能夠針對模型的關聯 (model relationships) 運作，甚至可以用它來定義。如果有個使用者，同時有許多篇發表的文章，而我們希望使用者帳號銷毀時，文章也跟著銷毀。於是，我們在 +User+ 模型中，針對 +User+ 模型跟 +Post+ 模型之間的關聯，加入 +after_destroy+ 回呼。範例如下：
...
回呼 (callbacks) 也可以寫成條件式的 (conditional) ，在某個先決條件成立時，才呼叫它們，就像本文前幾節在驗證 (validations) 中所做的一樣。要做條件判斷，可以用 +:if+ 跟 +:unless+ 選項 (options) ，這些選項可以接收符號 (symbol) 、字串 (string) 或是 +Proc+ 。定義條件時，若是在某狀況下 *必須* 呼叫回呼，可以用 +:if+ 選項；相反地，在某狀況下 *必須不* 呼叫回呼，則是用 +:unless+ 選項。

h4. 在 +:if+ 與 +:unless+ 中使用符號 (Symbol)

+:if+ 與 +:unless+ 可以用符號 (symbol) 來 associate ，這個符號 (symbol) ，是對應到回呼 (callback) 執行前一刻所呼叫的方法 (method) 的名稱。使用 +:if+ 選項 (option) 時，若這個方法傳回假值 (false) ，回呼就不會執行；相反地，使用 +:unless+ 選項的話，若這個方法傳回真值，回呼就不會執行。以上是最常用到的選項，用這種格式 (form) 註冊回呼的話，還可以針對回呼是否執行的判斷，註冊數個不同的方法。
...
條件式裡面也可以放字串 (string) ，這個字串會用 +eval+ evaluated，字串內容必須是一段有效的 Ruby 程式碼。只有真的很簡短的條件式，才適合用字串形式放在選項 (options) 裡面。
...
h4. 在 +:if+ 與 +:unless+ 中使用程式碼物件 (Proc)

最後， +:if+ 跟 +:unless+ 還可以用 +Proc+ 物件來 associate 。這種作法，尤其適合用來撰寫簡短的驗證方法，通常是一行以內的 (ne-liners) 。
...
有時候，某模型的回呼方法 (callback methods) ，在其他的模型中也能重複使用。 Active Record 讓我們能夠建立回呼類別 (callback classes) ，來封裝 (encapsulate) 這些回呼方法，如此一來，就可以很容易的重複利用它們。

底下範例，針對 +PictureFile+ 模型 (model) ，建立了一個內含 +after_destroy+ 回呼的類別 (class) 。
...
在類別內部宣告 (declare) 回呼方法 (callback methods) 的時候，回呼方法會接收模型物件 (model object) 做為參數 (parameter) ，用法如下：
...
回呼類別 (callback classes) 內，可宣告的回呼數量沒有限制，要多少個都可以。

h3. 觀測器 (Observers)

觀測器 (observers) 跟回呼 (callbacks) 很像，但是有個重大的差別，差別在於，回呼的程式碼會污染 (pollute) 模型，而這些程式碼，往往與模型 (model) 本身的用途，並沒有直接的關係；但是，觀測器 (observers) 卻可以在模型的外部，以不會污染到模型的方式，做到跟回呼一樣的功能。像是 +User+ 模型裡面，其實不該有送出註冊確認信 (registration confirmation emails) 的程式碼，這種時候，回呼內的程式碼跟模型本身沒有直接關係，就可以考慮用觀測器 (observer) 來代替回呼 (callbacks) 。

h4. 建立觀測器 (Creating Observers)

假設有個 +User+ 模型，希望每次它新增使用者的時候，系統都送出一封確認 email 。由於寄信並不是 +User+ 模型存在的目的，所以，我們可以用觀測器 (observer) 來加入這個功能。
...
觀測器的方法 (methods) ，將它所觀測的模型接收為一個參數 (parameter) ，這一點，就跟回呼類別 (callback classes) 是一樣的。
...
預設之下， Rails 會單純的把觀測器 (observer) 的名稱，除掉 "Observer" 這幾個字後，去尋找相同名稱的模型 (model) ，做為觀測的對象。不過，觀測器 (observer) 其實可以對上多個模型 (model) ，而我們可以手動的指定這些模型。
