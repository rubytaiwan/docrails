...
這一章說明如何 hook into Active Record 物件的生命週期：如何在物件進入資料庫之前，驗證它的狀態，以及，如何在物件生命週期的特定時間點，執行自訂的操作。
...
* 打造您自訂的驗證方法
* 處理驗證過程中所產生的錯誤訊息
* 在物件生命週期期間，會發生一些事件，您將學會針對這些事件撰寫 callback 方法。
* 在 callbacks （回呼）之間，有些共通的行為，您將學會針對這些行為撰寫特殊類別， 以封裝 (encapsulate) 它們。 
* 有些事件，並沒有定義在原來的類別中，您將學會針對這些事件，建立觀測器 (Observers)，以便回應它們。
endprologue.

h3. 物件生命週期 Object Life Cycle

在 Rails 的正常操作下，會產生一些物件，也會更新、銷毀它們。為此，Active Record 提供了 <em>object life cycle</em> 內的 hooks，讓您可以操控程式、操控資料。其中，驗證 (Validations) 的作用，是確保只有有效的資料，被存到資料庫裡面。而 Callbacks 和觀測器 (observers) 的作用，則是在物件狀態變化之前、或變化之後，觸發相關的邏輯。

h3. 驗證的概觀 Validations Overview

深入 Rails 的 validations 細節前，先了解一下 validations 扮演的角色。

h4. 為什麼要驗證 Validations?

Validations 用來確保你的資料庫裡面，只有有效資料 (valid data) 可以存進去。舉個例子，你的應用程式，可能要確定每個使用者，都提供有效的電子郵件地址和郵寄地址。

在存進資料庫前，有幾個驗證資料的方法，包括：native database constraints, client-side validations, controller-level validations, 以及 model-level validations.

* Database constraints and/or 預存程序 stored procedures，這些驗證的機制要依賴特定的資料庫，所以，測試和維護上會比較難。不過，如果資料庫會去給別的應用程式用，那麼在資料庫層級設定 constraints 可能就是不錯的作法。另外，資料庫層級的驗證可以很安全的處理一些事情，像是在重度使用的資料表中，確保唯一值之類的。這些事情，如果用其他的方法，會比較難實作。
* 客戶端的驗證 Client-side validations 雖然有用，但單單用這個的話，通常不太可靠。如果用 JavaScript 來做，要是 user 的瀏覽器把 JavaScript 關掉，那驗證就被略過了。不過，畢竟 client-side validation 可以很即時的給使用者回饋，如果跟其他的驗證手法併用，那倒是挺方便的。
* Controller-level validations 是也能用，但常常會變得很笨重，不好測試跟維護。不管怎樣，"keep your controllers skinny":http://weblog.jamisbuck.org/2006/10/18/skinny-controller-fat-model 總是好的，長遠的說，可以讓程式用起來很愉快。
* Model-level validations 是驗證資料的最佳作法。這種驗證方式，不用去管資料庫是哪個系統，也不會被使用者略過，測試起來很方便，也容易維護。為了方便大家使用 model 層級的驗證，Rails 針對常用功能內建了輔助工具，也讓您可以撰寫自己的 validation methods。

h4. 什麼時候會發生 Validation ？

Active Record objects 有兩種：有跟某一列資料對應的，或者沒有跟資料對應的。當您使用 +new+ 之類的方法，建立了一個新物件，此時，它還不屬於資料庫。Once you call +save+ upon that object it will be saved into the appropriate database table. Active Record uses the +new_record?+ instance method to determine whether an object is already in the database or not. Consider the following simple Active Record class:
