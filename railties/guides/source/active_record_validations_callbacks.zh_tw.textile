...
h4(#working_with_validation_errors-errors). +errors+ （列出全部錯誤訊息）
...
h4(#working_with_validation_errors-errors-2). +errors[]+ （列出某個屬性的全部錯誤訊息）
...
除了 +add+ 之外的另一個作法，是用 +[]=+ 這個 ??? 設定函式 (setter)
