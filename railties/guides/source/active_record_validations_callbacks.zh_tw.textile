...
新記錄 (record) 的新增與儲存，會對資料庫送出 SQL 的 +INSERT+ 操作 (operation) 。現有記錄的更新，則會送出 +UPDATE+ 操作。通常，在這些命令送到資料庫之前，會進行驗證，如果任一個驗證失敗的話，這個物件就標記為無效 (marked as invalid) ，於是 Active Record 就不會執行 +INSERT+ 或 +UPDATE+ ，所以，無效的物件也就不會存進資料庫。您也可以針對物件的新增、儲存、更新，指派某幾個特定的驗證。
