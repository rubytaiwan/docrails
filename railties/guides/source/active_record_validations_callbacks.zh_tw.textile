...
h4. +validates_acceptance_of+ （驗證使用者是否已接受）
...
h4. +validates_confirmation_of+ （驗證使用者的再次確認）
...
h4. +validates_exclusion_of+ （驗證屬性的值不被包括在內）
...
h4. +validates_format_of+ （驗證屬性值的格式）
...
h4. +validates_inclusion_of+ （驗證屬性的值有被包括在內）
...
h4. +validates_length_of+ （驗證長度是否在指定範圍內）
...
h4. +validates_numericality_of+ （驗證屬性的值只包括數值）
